...
public class MedicineTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int lines, int columns) {
        ...
        if (columns == MEDICINE_CODE_COL) {
            return medicine.getCode();
        }
        if (columns == MEDICINE_NAME_COL) {
            return medicine.getName();
        }
        if (columns == MEDICINE_DESCRIPTION_COL) {
            return medicine.getDescription();
        }
        if (columns == MEDICINE_COMPOSITION_COL) {
            return medicine.getComposition();
        }
        if (columns == MEDICINE_PRICE_COL) {
